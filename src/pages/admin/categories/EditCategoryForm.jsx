...
export default function EditCategoryForm({ categoryId, onCancel, onUpdated }) {
    ...
    const load = async () => {
      ...
      try {
        ...
        reset({
          ...
          status: c?.status ?? c?.enabled ?? c?.active ?? 'Active',
        })
      } catch (e) {
        if (!alive) return
        setSubmitError(e?.message || 'Unable to load category')
      } finally {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      setSubmitError(e?.message || 'Unable to update category')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (loading) {
    return <div className="rounded-3xl border border-border bg-background p-10 text-center text-sm font-bold text-muted">Loading...</div>
  }
