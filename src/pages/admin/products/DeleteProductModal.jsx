...
export default function DeleteProductModal({ productId, onCancel, onDeleted }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!alive) return
        setProductName(p?.name ?? p?.productName ?? 'this product')
      } catch {
        if (!alive) return
  ...
  return (
    <ModalShell title="Delete Product" onClose={onCancel}>
      <div className="text-sm font-semibold text-muted">
        Are you sure you want to delete <span className="font-extrabold text-ink">{loading ? '...' : productName}</span>? This action cannot be undone.
      </div>
